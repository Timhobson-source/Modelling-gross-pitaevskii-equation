...
u0=A*sech(A*(x-4-(R+L)/2)); % unpeturbed initial guess in centre of domain
u0=u0';
rng('default');
pert=1*0.5; %size of perturbation
up=u0+pert*(rand(Nx,1)-0.5); %peturbed IC. rand(a,b) returns a a-by-b array of random numbers
...
while(err>1e-6) %1e-6 is our error tolerance.
    ...
    plot(x,U(1:Nx),'.',x,U(Nx+1:end),'.',x,U1(1:Nx),x,U1(Nx+1:end),x,V,'--')
    title(['it=',num2str(it)]);
    legend('Re(previous)','Im(previous)','Re(current)','Im(current)','Potential','Location','NorthEast') 
